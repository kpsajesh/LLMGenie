# utils contains the generic methods which can be used across the project
...
import requests # this is for the api calls

# This method is to take the requirement file from the directory
def pick_requirement(path_arg: str | None, req_dir: Path) -> Path:
    ...
    """
    #print(f"pick_requirement path_arg- {path_arg}")
    if path_arg:
        p = Path(path_arg)
        if not p.exists():
...
# We get response as json from LLM, this method is to parse the json response
...
# This method is to convert the json response to rows which can be written to csv
def to_rows(cases: List[Dict]) -> List[List[str]]:
    ...
    """
    rows: List[List[str]] = [] # rows variable declarion with type hint, variable contains list inside list
...
# This method is to write the rows to a csv file
...
# This method is to write the json object to a file
...
# To consume apis, so this method is to make http post request with url, json payload, headers (sometimes can be empty) and timeout) and timeout
...
# To consume apis, so this method is to make http get request with url, headers (sometimes can be empty) and timeout. No payload, it is get request
